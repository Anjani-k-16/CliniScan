...
# --- Configuration ---
# Setting device to CPU is necessary for this environment
...
# Transformation for Grad-CAM (simple resize and ToTensor, no normalization yet)
transform_gradcam = transforms.Compose([
    transforms.Resize((224, 224)),
    transforms.ToTensor(),
])

# Standard transformation for model inference (resize, ToTensor, and normalization)
...
def generate_grad_cam(model, target_layer, img_tensor, original_img):
    ...
    feature_maps = []
    # Hook to capture the forward pass output (feature maps)
    def forward_hook(module, input, output):
        feature_maps.append(output)
    
    gradients = []
    # Hook to capture the backward pass output (gradients)
...
st.title("CHEST X-RAY CLASSIFIER (PyTorch Only)") 
st.markdown("---")
...
if uploaded_file:
    ...
    # --- PYTORCH PREDICTION (Now the only prediction) ---
    ...
    if pred_class_pt == "PNEUMONIA":
        ...
        st.info("Grad-CAM visualization skipped as the diagnosis is NORMAL.")
    ...
    # --- PYTORCH CHART (Now the only chart) ---
